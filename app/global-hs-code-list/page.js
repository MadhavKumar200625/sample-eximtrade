...
const page = async () => {
      ...
      const result = await db.query`
        select hs_code, HS_code_Description from hscode_list where len(hs_code) = 2 order by hs_code
          ...
          <div className="overflow-x-auto mt-10 border border-gray-300  shadow-md">
            <table className="min-w-full text-left text-black border-collapse">
                ...
                <tr className="bg-gray-200">
                  ...
                  <th className="px-6 py-4 text-base font-semibold border-b w-[60%] border-gray-300 border-l">
                    Chapter Description
                  </th>
                ...
                {result.recordset.map((row, idx) => (
                  ...
                  >
                    <td className="px-6 py-4 font-semibold border-b border-gray-200">
                      <div className="block w-full">chapter {row.hs_code}</div>
                    </td>
                    <td className="px-6 py-4 border-b border-gray-200 border-l">
                      <div className="block w-full">{row.HS_code_Description}</div>
                    </td>
                    <td className="px-4 py-4 border-b border-gray-200 border-l">
                      <Link href={`/search/country-India/type-import/hscode-${row.hs_code}`} className="block bg-blue-600 text-white p-2 text-x cursor-pointer underline text-center w-full">Chapter - {row.hs_code}</Link>
                    </td>
                    <td className="px-4 py-4 border-b border-gray-200 border-l">
                      <Link href={`/search/country-India/type-import/hscode-${row.hs_code}`} className="block text-center bg-gray-400 p-2 text-white underline cursor-pointer  w-full">Chapter - {row.hs_code}</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
